...
@Component({
  ...
  templateUrl: './latepayment-review.component.html',
  styleUrls: ['./latepayment-review.component.less']
})
export class LatepaymentReviewComponent implements OnInit, OnDestroy {

  public latepaymentReviewContent: any;
  ...
  constructor(
    ...
    this.themeSub =  this.themeSrv.changeEmitted$.subscribe(
      value => {
        ...
        this.setTableOption(this.latepaymentReviewContent);
      }
    );
  ...
  public reviewClick(): void {
    if (this.latereviewSelect.length === undefined || this.latereviewSelect.length === 0) {
      this.toolSrv.setToast('error', '操作错误', '请选择一项进行审核');
    } else if (this.latereviewSelect.length === 1) {
      this.reviewOption = {
        width: '500',
        dialog: true
      };
  ...
  public  queryData(): void {
    ...
    this.lateSrv.queryLatePaymentReviewPageData(this.SearchData).subscribe(
      value => {
        // console.log(value);
        this.loadHidden = true;
        if (value.status === '1000') {
          this.latepaymentReviewContent = value.data.contents;
  ...
  public  reviewPassClick(e): void {
      ...
      if (e === '通过') {
        ...
        // 审核不通过时的请求
        this.lateSrv.reviewLatePaymentNoPass({id: this.latereviewSelect[0].id}).subscribe(
          value => {
            this.toolSrv.setQuestJudgment(value.status, value.message, () => {
              this.latereviewSelect = [];
              this.reviewOption.dialog = false;
